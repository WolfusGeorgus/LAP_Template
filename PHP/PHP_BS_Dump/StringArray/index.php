    ...
    <div class="container mt-5">
        ...
        <form method="post" action="index.php">
            <div class="form-group">
                <label for="textInput">Geben Sie Ihren Text ein:</label>
                <textarea class="form-control" id="textInput" name="text" rows="5"><?php if (isset($_POST['text'])) { echo htmlspecialchars($_POST['text']); } ?></textarea>
            </div>
        ...
        if (isset($_POST['text'])) {
            ...
            $text = $_POST['text'];
            $words = str_word_count($text, 1, $diacritics);
            $characterCount = strlen($text);
            $wordCount = count($words);
            $wordCountOccurrences = array_count_values($words);
            $duplicateCount = 0;

            echo "<div class='mt-3'>";
            echo "<p class='result-line'>Anzahl der Buchstaben: $characterCount</p>";
            echo "<p class='result-line'>Anzahl der Wörter: $wordCount</p>";

            // Zähle die doppelten Wörter
            foreach ($wordCountOccurrences as $word => $count) {
                if ($count > 1) {
                    $duplicateCount++;
                }
            }
            ...
            if ($duplicateCount > 0) {
              echo "<p class='mb-1'>Anzahl der doppelten oder mehrfachen Wörter: $duplicateCount</p>";
                foreach ($wordCountOccurrences as $word => $count) {
                    if ($count > 1) {
                        echo "<p class='mb-1'>'$word' kommt $count-mal vor</p>";
                    }
                }
            } else {
                echo "<p>Der Text enthält keine Wörter mehrfach.</p>";
            }
            echo "</div>";
        }
        ?>
